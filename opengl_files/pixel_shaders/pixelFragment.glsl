...
vec3 random_in_unit_sphere(vec3 p) {
  return normalize(vec3(rand(p.xy ), rand(p.yz), rand(p.zy)));
}

//glsl recursion desteklemedigi icin kendisini cagiramiyoruz
//bu nedenle tek bir depth hesaplanir
...
void main(){
    ...
    vec3 origin 	=	vec3(.1 + cos(iTime * 2.) * .05,  .01,-.1 - sin(iTime) * .3);
    ...
    sphere[3] spheres = sphere[3](
        sphere(vec3(0.0,0.0,-1.), .25, vec3(.2, 0.1, 0.1), 1),
        sphere(vec3(-0.5, 0.0, -1.2), .25, vec3(.0, 0.0, 0.5), 0),
        sphere(vec3(0.0,-100.25,-1.), 100.00, vec3(0.2,0.4,0.2), 0)
    );
    
    vec3 result = vec3(0.0, 0.0, 0.0);

    //pixel basina sample sayisi
    int N = 25;
